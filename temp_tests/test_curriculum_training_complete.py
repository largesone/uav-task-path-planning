# -*- coding: utf-8 -*-
"""
文件名: test_curriculum_training_complete.py
描述: 课程学习训练协调器完整测试代码（修复版）
作者: AI Assistant
日期: 2024
...
class TestCurriculumStage(unittest.TestCase):
    ...
    def test_curriculum_stage_creation(self):
        ...
        def dummy_scenario(obstacle_tolerance=50.0):
            return [], [], []  # 返回空的uavs, targets, obstacles
...
class TestCurriculumTrainer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 简化课程阶段用于测试
        def dummy_scenario(obstacle_tolerance=50.0):
            # 正确的UAV构造函数调用：id, position, heading, resources, max_distance, velocity_range, economic_speed
    ...
    @patch('main.run_scenario')  # 修复导入路径
