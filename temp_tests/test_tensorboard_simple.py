...
class TestCurriculumTensorBoardWriterSimple(unittest.TestCase):
    """测试课程学习TensorBoard写入器核心功能"""
    ...
    @patch('torch.utils.tensorboard.SummaryWriter.add_scalar')
    def test_log_attention_weights(self, mock_add_scalar, mock_add_histogram, 
                                 mock_add_figure, mock_subplots, mock_close):
        """测试注意力权重记录"""
        # 设置matplotlib模拟
        fig_mock = Mock()
        ax_mock = Mock()
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('torch.utils.tensorboard.SummaryWriter.add_figure')
    def test_log_learning_curves(self, mock_add_figure, mock_subplots, mock_close):
        """测试学习曲线记录"""
        # 设置matplotlib模拟
        fig_mock = Mock()
        ax_mock = Mock()
...
class TestTensorBoardCustomPluginSimple(unittest.TestCase):
    """测试TensorBoard自定义插件"""
...
class TestTensorBoardIntegrationSimple(unittest.TestCase):
    """简化版TensorBoard集成测试"""
    ...
    def test_complete_tensorboard_workflow(self):
        ...
        with patch('matplotlib.pyplot.subplots') as mock_subplots, \
             patch('matplotlib.pyplot.close'), \
             patch.object(writer.writer, 'add_figure'), \
...
def run_simple_tensorboard_tests():
    """运行简化版TensorBoard集成测试"""
    print("📊 开始简化版TensorBoard集成测试...")
    print("=" * 50)
    ...
    # 添加测试类
    test_classes = [
        TestCurriculumTensorBoardWriterSimple,
        TestTensorBoardCustomPluginSimple,
        TestTensorBoardIntegrationSimple
    ]
...
if __name__ == "__main__":
    success = run_simple_tensorboard_tests()
