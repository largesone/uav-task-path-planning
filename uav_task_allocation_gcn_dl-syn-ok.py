...
def visualize_task_assignments(final_plan, uavs, targets, show=True):
    """
    【v5.1 最终显示增强版】可视化函数，在图上直接标注速度建议和差值。
    """
    set_chinese_font()
    ...
    for uav_id, tasks in final_plan.items():
        ...
        for i, task_info in enumerate(tasks):
            ...
            # --- 【新增部分】在图上标注速度和差值 ---
            speed_diff = task_info['speed'] - uav.economic_speed

            # 创建标注文本，分两行显示
            annotation_text = f"{task_info['speed']:.1f} m/s\n({speed_diff:+.1f})"

            # 在路径中点下方添加标注
            ax.annotate(annotation_text,
                        xy=mid_point,
                        xytext=(0, -25),  # 文本位于中点下方25个点的位置
    ...
    for uav in uavs:
        ...
        if not details:
            ...
            for detail in details:
                speed_diff = detail['speed'] - uav.economic_speed
                diff_str = f"{speed_diff:+.2f}"
                sync_status = "" if detail['is_sync_feasible'] else " (警告: 无法满足同步)"
                report_text += f"  {detail['step']}. 飞向目标 {detail['target_id']}{sync_status}:\n"
                report_text += f"     飞行速度: {detail['speed']:.2f} m/s (与经济速度差: {diff_str} m/s)\n"
...
def main():
    # (UAV和Target定义，以及RL模型训练和评估部分的代码保持不变)
    uavs = [
    ...
    for uav_id in sorted(final_plan.keys()):
        ...
        if tasks:
            ...
            for task in tasks:
                # --- 新增计算 ---
                speed_diff = task['speed'] - uav.economic_speed
                diff_str = f"{speed_diff:+.2f}"
                ...
                # --- 新增显示 ---
                print(
                    f"  - 第 {task['step']} 步: 飞向目标 {task['target_id']}{sync_status}, 飞行速度: {task['speed']:.2f} m/s (差值: {diff_str} m/s), 到达时间点: {task['arrival_time']:.2f}s")
        else:
